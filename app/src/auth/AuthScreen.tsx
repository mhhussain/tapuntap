...
const pwStrength = (s: string): number => {
  let score = 0;
  if (s.length >= 8) score++;
...
function StrengthMeter({ score }: { score: number }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      <div style={{ fontSize: 11, color: colors[score], marginTop: 4, fontFamily: "var(--font-mono)" }}>
        {labels[score]}
      </div>
    </div>
  );
...
function AuthField({ label, error, aside, children }: AuthFieldProps) {
  return (
    <div>
...
export function AuthScreen() {
  ...
  // Field-level validation (only after blur / submit attempt)
  const emailErr = touched.email && !emailValid ? "Enter a valid email address." : null;
  const pwErr =
    touched.pw && mode === "signup" && pw && strength < 2
    ...
    <div className="auth-bg" style={{ display: "grid", placeItems: "center", height: "100vh", padding: 24, overflowY: "auto" }}>
      <div style={{ width: "100%", maxWidth: 396, display: "flex", flexDirection: "column", gap: 22 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, marginBottom: 4 }}>
          <div style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 36, color: "var(--fg-0)", lineHeight: 1 }}>
            tapuntap
          </div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", background: "var(--bg-0)", border: "1px solid var(--line-1)", borderRadius: 8, padding: 3, marginBottom: 20 }}>
            {(["signin", "signup"] as const).map((m) => (
              <button key={m} onClick={() => switchMode(m)} style={{
                ...
                fontWeight: 600, fontSize: 13, transition: "all 120ms", cursor: "pointer",
              }}>
          ...
          {/* Google */}
          <button className="btn" onClick={handleGoogleAuth} disabled={googleLoading || submitting}
            style={{ width: "100%", justifyContent: "center", padding: "10px 0", fontWeight: 600, fontSize: 14, background: "var(--bg-2)", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <AuthField
              ...
              error={pwErr}
              aside={mode === "signin" ? (
                <button type="button" className="auth-link" title="Coming soon" onClick={(e) => e.preventDefault()}>
                  Forgot password?
                </button>
              ) : undefined}
            >
            ...
            <button type="submit" className="btn btn-primary" disabled={submitting || googleLoading}
              style={{ width: "100%", justifyContent: "center", padding: "10px 0", fontSize: 14, marginTop: 2, display: "flex", alignItems: "center", gap: 8 }}>
